...
export default function FriendsContainer({ children }: { children: React.ReactNode }) {
  const pathname = usePathname();
  const sidebarItems = [
    { path: 'ban-be', label: 'Tất cả bạn bè', icon: Users },
            ...
            {sidebarItems.map(({ path, label }) => (
              <Link
                ...
                className={`flex-shrink-0 px-4 py-2 mr-2 rounded-full ${
                  pathname === `/ban-be/${path}` ? 'bg-blue-50 text-blue-600' : 'bg-gray-100'
                }`}
              >
                {label}
              </Link>
