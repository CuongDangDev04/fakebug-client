...
export const friendshipService = {
    ...
    async sendFriendRequest(receiverId: number) {
        return axiosInstance.post(`${BASE_URL}/send-request/${receiverId}`);
    },

    // Phản hồi lời mời kết bạn
    async respondToRequest(requestId: number, accept: boolean) {
        return axiosInstance.post(`${BASE_URL}/respond/${requestId}`, { accept });
    },

    // Lấy danh sách bạn bè
    async getFriends() {
        return axiosInstance.get(`${BASE_URL}/friends`);
    },

    // Hủy kết bạn
    async unfriend(targetId: number) {
        return axiosInstance.delete(`${BASE_URL}/unfriend/${targetId}`);
    },

    // Chặn người dùng
    async blockUser(targetId: number) {
        return axiosInstance.post(`${BASE_URL}/block/${targetId}`);
    },

    // Bỏ chặn người dùng
    async unblockUser(targetId: number) {
        return axiosInstance.delete(`${BASE_URL}/unblock/${targetId}`);
    },

    // Lấy danh sách lời mời kết bạn đã nhận
    async getReceivedRequests() {
        return axiosInstance.get(`${BASE_URL}/requests/received`);
    },

    // Lấy danh sách lời mời kết bạn đã gửi
    async getSentRequests() {
        return axiosInstance.get(`${BASE_URL}/requests/sent`);
    },

    // Hủy lời mời kết bạn đã gửi
    async cancelSentRequest(targetId: number) {
        return axiosInstance.delete(`${BASE_URL}/requests/cancel/${targetId}`);
    },

    //lấy danh sách bạn chung
    async getMutualFriends(targetId: number) {
        return axiosInstance.get(`${BASE_URL}/mutual/${targetId}`);
    },
    ...
    async checkFriendshipStatus(targetId: number) {
        return axiosInstance.get<FriendshipStatus>(`${BASE_URL}/status/${targetId}`);
    },

    // Lấy danh sách bạn bè của một user cụ thể
    async getUserFriends(userId: number) {
        return axiosInstance.get(`${BASE_URL}/user/${userId}/friends`);
    },
